...
const formatLogger = (messageType) => {
    return inputMess = (messageInp) => {
        return `${messageType}: ${messageInp}`;
    }
};
...
const calculateResult = (num1, num2, callback1, callback2) => {
    let result;
    callback1(num1, num2, (a, b) => {
        if (isNaN(a) || isNaN(b)) result = ('Error with numbers. Operation impossible');
        else result = callback2(num1, num2);
    });
    return result;
}
